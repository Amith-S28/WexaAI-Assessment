# Graph Report - Wexa AI Graph Database Benchmark Suite  (2026-08-21)

## Corpus Check
- Large corpus: 124 files · ~825,435 words. Semantic extraction will be expensive (many Claude tokens). Consider running on a subfolder.

## Summary
- 323 nodes · 565 edges · 23 communities (19 shown, 4 thin omitted)
...
- Module: scripts_export_pdf, scripts_export_pdf_export_html_to_pdf, scripts_export_pdf_find_browser
...
- Module: scripts_test_connections, scripts_test_connections_main, scripts_test_connections_test_bolt_connection
...
Cohesion: 0.22
Nodes (3): JanusGraphAdapter, Any, Adapter for JanusGraph distributed graph database via TinkerPop Gremlin HTTP end
...
Cohesion: 0.18
Nodes (6): clean_key_name(), MasterReportGenerator, Any, Path, Master Diagram and Benchmark Report Generator. Merges Cloud Run CognoDB baseline, run()
...
### Community 11 - "Module: scripts_export_pdf, scripts_export_pdf_export_html_to_pdf, scripts_export_pdf_find_browser"
Cohesion: 0.23
Nodes (16): export_html_to_pdf(), find_browser(), main(), Path, Export HTML Benchmark Report to High-Fidelity Publication-Grade PDF Uses Headles, verify_pdf_contents(), build_executive_html(), generate_cloud_charts() (+8 more)

### Community 12 - "Module: abc, benchmarks_adapters_arangodb, benchmarks_adapters_arcadedb"
Cohesion: 0.17
Nodes (8): ABC, ArcadeDB Adapter. Implements BaseGraphAdapter for ArcadeDB Multi-Model Graph Eng, Base Graph Database Adapter Interface. Defines the standard contract for all dat, FalkorDB Cloud Adapter. Implements BaseGraphAdapter for FalkorDB's GraphBLAS spa, Database Adapters Package., JanusGraph Adapter. Implements BaseGraphAdapter for JanusGraph (Apache TinkerPop, KùzuDB Adapter. Implements BaseGraphAdapter for KùzuDB (In-Process / Columnar Gr, Workload Execution Engine. Executes standard benchmark workloads across database

### Community 13 - "Module: benchmarks_adapters_base_basegraphadapter_aggregate_degree_distribution, benchmarks_adapters_base_basegraphadapter_bulk_insert_edges, benchmarks_adapters_base_basegraphadapter_bulk_insert_nodes"
Cohesion: 0.13
Nodes (8): Any, Ingest node records in batches.         Returns dict with: total_nodes, elapsed_, Ingest edge records in batches.         Returns dict with: total_edges, elapsed_, Lookup node by ID. Returns (latency_ms, record_dict)., Filter nodes by category. Returns (latency_ms, list_of_records)., Aggregate top-N nodes by out-degree. Returns (latency_ms, list_of_top_nodes)., Execute a mixed read/write transaction. Returns (latency_ms, success)., Inspect storage/memory footprint where observable.

### Community 14 - "Module: benchmarks_adapters_bolt_base, benchmarks_adapters_bolt_base_rationale_1, benchmarks_adapters_cognodb"
Cohesion: 0.25
Nodes (4): Bolt-based Base Adapter for Cypher-compatible graph databases: - CognoDB Cloud (, CognoDB Cloud Adapter. Implements BoltGraphAdapter for Wexa AI's CognoDB Cloud p, Memgraph Cloud Adapter. Implements BoltGraphAdapter for Memgraph in-memory C++ g, Neo4j AuraDB Adapter. Implements BoltGraphAdapter for Neo4j AuraDB Cloud.
...
Cohesion: 0.50
Nodes (3): Path, Graphify Pipeline Execution Harness Executes graphify Steps 1-5 on the repositor, run_pipeline()
...
- **Why does `BoltGraphAdapter` connect `Module: benchmarks_adapters_bolt_base_boltgraphadapter, benchmarks_adapters_bolt_base_boltgraphadapter_aggregate_degree_distribution, benchmarks_adapters_bolt_base_boltgraphadapter_bulk_insert_edges` to `Module: benchmarks_adapters_cognodb_cognodbadapter, benchmarks_adapters_cognodb_cognodbadapter_init, benchmarks_adapters_cognodb_rationale_9`, `Module: benchmarks_adapters_base_basegraphadapter, benchmarks_adapters_base_basegraphadapter_close, benchmarks_adapters_base_basegraphadapter_connect`, `Module: abc, benchmarks_adapters_arangodb, benchmarks_adapters_arcadedb`, `Module: benchmarks_adapters_bolt_base, benchmarks_adapters_bolt_base_rationale_1, benchmarks_adapters_cognodb`?**
...
- **Are the 6 inferred relationships involving `BaseGraphAdapter` (e.g. with `ArangoDBAdapter` and `ArcadeDBAdapter`) actually correct?**
  _`BaseGraphAdapter` has 6 INFERRED edges - model-reasoned connections that need verification._
