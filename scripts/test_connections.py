...
def main():
    ...
    console.print(f"[bold cyan]Testing FalkorDB Cloud ({falkor_host}:{falkor_port})...[/bold cyan]")
    if falkor_host and "localhost" not in falkor_host:
        try:
            from falkordb import FalkorDB
            t0 = time.perf_counter_ns()
            client = FalkorDB(host=falkor_host, port=falkor_port, username=falkor_user, password=falkor_pwd)
            g = client.select_graph("benchmark_test")
            res = g.query("RETURN 1 AS ping")
            t1 = time.perf_counter_ns()
            rtt_ms = (t1 - t0) / 1_000_000
            val = res.result_set[0][0]
            console.print(f"[bold green][OK] FalkorDB Cloud connected successfully! RTT: {rtt_ms:.2f} ms (Result: {val})[/bold green]")
            results.append(("FalkorDB Cloud", f"{falkor_host}:{falkor_port}", "Connected", f"{rtt_ms:.2f} ms"))
        except Exception as e:
            console.print(f"[bold red][FAIL] FalkorDB Cloud connection failed: {e}[/bold red]")
            results.append(("FalkorDB Cloud", f"{falkor_host}:{falkor_port}", "Failed", str(e)))
    else:
        results.append(("FalkorDB Cloud", "Not configured", "Failed", "Localhost placeholder"))
    ...
    console.print(f"[bold cyan]Testing ArangoDB Oasis Cloud ({arango_url})...[/bold cyan]")
    if arango_url and "localhost" not in arango_url:
        try:
            ...
            t1 = time.perf_counter_ns()
            rtt_ms = (t1 - t0) / 1_000_000
            console.print(f"[bold green][OK] ArangoDB Oasis Cloud connected successfully! RTT: {rtt_ms:.2f} ms (Version: {ver})[/bold green]")
            results.append(("ArangoDB Oasis Cloud", arango_url, "Connected", f"{rtt_ms:.2f} ms (v{ver})"))
        except Exception as e:
            console.print(f"[bold red][FAIL] ArangoDB Oasis Cloud connection failed: {e}[/bold red]")
            results.append(("ArangoDB Oasis Cloud", arango_url, "Failed", str(e)))
    else:
        results.append(("ArangoDB Oasis Cloud", "Not configured", "Failed", "Localhost placeholder"))
